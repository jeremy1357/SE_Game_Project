...
void FPS_Timer::end_FPS()	//This Funciton ends the timer and calculates how long the the updates were
{
	...
	std::chrono::milliseconds target(std::chrono::milliseconds(1000)/std::chrono::duration_cast<std::chrono::milliseconds>(m_setFPS));
	const duration<double> sleep = target - m_fps_timer;

	if (m_fps_timer < target)	//if the time is less than 17 milliseconds, then sleep to get to 17 milliseconds (~60 fps)
	{
